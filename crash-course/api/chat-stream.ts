...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!apiKey) {
      ...
      return res.json({ response: fallback, isOnline: false });
    }
    ...
    // If headers haven't been sent yet, return JSON fallback
    if (!res.headersSent) {
      const fallback = FALLBACK_RESPONSES[Math.floor(Math.random() * FALLBACK_RESPONSES.length)];
      return res.json({ response: fallback, isOnline: false, error: 'Failed to get AI response' });
    }
